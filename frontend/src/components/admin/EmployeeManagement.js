...
const EmployeeManagement = () => {
    ...
    useEffect(() => {
        if (selectedDepartment) {
            fetchSubdepartmentsByDepartment(selectedDepartment);
        } else {
            setSubdepartments([]);
    ...
    const fetchEmployees = async (departmentId = null) => {
        try {
            let url = 'http://localhost:8080/api/admin/accounts/employees';
            if (departmentId) {
    ...
    const fetchDepartments = async () => {
        try {
            const response = await axios.get(
                'http://localhost:8080/api/admin/accounts/employees/departments',
                { withCredentials: true }
            );
    ...
    const fetchSubdepartmentsByDepartment = async (departmentId) => {
        try {
            const response = await axios.get(
                `http://localhost:8080/api/admin/accounts/employees/subdepartments?departmentId=${departmentId}`,
                { withCredentials: true }
            );
            setSubdepartments(response.data);
        } catch (err) {
            console.error('Ошибка загрузки кафедр:', err);
            setError('Не удалось загрузить список кафедр');
        }
    };
    ...
    const fetchPositions = async () => {
        try {
            const response = await axios.get(
                'http://localhost:8080/api/admin/accounts/employees/positions',
                { withCredentials: true }
            );
    ...
    const fetchRoles = async () => {
        try {
            const response = await axios.get(
                'http://localhost:8080/api/admin/accounts/employees/roles',
                { withCredentials: true }
            );
    ...
    const handleDeleteEmployee = async (id) => {
        if (window.confirm('Вы уверены, что хотите удалить этого сотрудника?')) {
            try {
                setProcessing(true);
                await axios.delete(
                    `http://localhost:8080/api/admin/accounts/employees/${id}`,
                    { withCredentials: true }
                );
    ...
    const handleSubmit = async () => {
        ...
        if (!employeeForm.idDepartment) errors.department = "Факультет обязателен";
        ...
        try {
            ...
            const formData = {
                ...employeeForm,
                idDepartment: employeeForm.idSubdepartment || employeeForm.idDepartment
            };
            ...
            const response = isEditMode
                ? await axios.put(
                    `http://localhost:8080/api/admin/accounts/employees/${currentEmployee.idEmployee}`,
                    formData,
                    config
                )
                : await axios.post(
    ...
    return (
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', marginBottom: '20px' }}>
                ...
                <div style={{ flex: 1 }}>
                    <label style={{ marginRight: '10px', fontSize: '1.1rem' }}>Фильтр по кафедре:</label>
                    <select
                        value={selectedSubdepartment || ''}
                        onChange={(e) => setSelectedSubdepartment(e.target.value || null)}
                    ...
                    >
                        ...
                        {subdepartments.map(subdepartment => (
                            <option key={subdepartment.idDepartment} value={subdepartment.idDepartment}>
                                {subdepartment.name}
                            </option>
                        ))}
                    </select>
            ...
            {employees.length > 0 ? (
                ...
                <p style={{ fontSize: '1rem', color: '#7f8c8d' }}>Список сотрудников пуст</p>
            )}
                    ...
                    <div style={{
                        ...
                        <form>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                ...
                                <select
                                    name="idSubdepartment"
                                    value={employeeForm.idSubdepartment || ''}
                                    onChange={handleFormChange}
                                    style={{
                                        width: '100%',
                                        borderRadius: '6px',
                                    ...
                                    disabled={processing || !employeeForm.idDepartment}
                                >
