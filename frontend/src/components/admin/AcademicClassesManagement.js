...
const AcademicClassesManagement = () => {
    ...
    const fetchClasses = async () => {
        try {
            let url = `http://localhost:8080/api/admin/schedule/classes?curriculumId=${selectedCurriculum}&semesterId=${selectedSemester}`;
    ...
    const handleDeleteClass = async (id) => {
        if (window.confirm('Вы уверены, что хотите удалить это занятие?')) {
            try {
                setProcessing(true);
                await axios.delete(
                    `http://localhost:8080/api/admin/schedule/classes/${id}`,
                    { withCredentials: true }
                );
                await fetchClasses();
            } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = isEditMode
                ? await axios.put(
                    `http://localhost:8080/api/admin/schedule/classes/${currentClass.idClass}`,
                    data,
                    config
                )
                : await axios.post(
                    'http://localhost:8080/api/admin/schedule/classes',
                    data,
                    config
                );
