...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/student/schedule/today', {
                    ...
                    {schedule.length > 0 ? (
                        <div style={{
                            ...
                            {schedule.map((item, index) => {
                                ...
                                const endTime = formatTime(new Date(item.datetime).setHours(new Date(item.datetime).getHours() + 1, 30));
